...
app.post('/:cid/product/:pid',(req,res)=>{
  ...
  if (ManagerCarrito.carritos.filter(e => e.id===cid)) {
    return res.json({message:"El carrito ya existe"})
  }else{
    ManagerCarrito.addCarrito(cid)
    ManagerCarrito.addProductsToCart(pid)
    res.status(201).json({message:"Porducto agregado correctamente"})
  }
